    ...
    class Program
    {
        ...
        public static decimal GetPrice(ExType type, string marketid, int count = 20)//20
        {
            ...
            if (type == ExType.Buy)
            {
                for (var i = 0; i < 20; i++)
                {
                    amountBids += depth.Bids.ElementAt(i).Value.Amount;
                    //floatamountbuy就是预设的累计买单深度
                    if (amountBids > floatamountbuy)
                    {
                        //稍微加0.01，使得订单排在前面
                        return depth.Bids.ElementAt(i).Value.Price + 0.01m;
                    }
                }
            }
            //同理计算卖价
            if (type == ExType.Sell)
            {
                for (var j = 0; j < 20; j++)
                {
                    amountAsks += depth.Asks.ElementAt(j).Value.Amount;
                    if (amountAsks > floatamountsell)
                    {
                        return depth.Asks.ElementAt(j).Value.Price - 0.01m;
                    }
                }
            }
            //遍历了全部深度仍未满足需求，就返回一个价格，以免出现bug
            return depth.Asks.ElementAt(0).Value.Price;
        }
